...
const Sidebar = () => {
  ...
  return (
    <aside className={`h-full ${collapsed ? 'w-16' : 'w-20 lg:w-72'} border-r border-base-300 flex flex-col transition-all duration-300`}>
      
      {/* Sidebar header */}
      <div className="border-b border-base-300 w-full p-5 flex justify-between items-center">
        <div className="flex items-center gap-2">
          <Users className="size-6" />
          {!collapsed && <span className="font-medium hidden lg:block">Contacts</span>}
        </div>
      ...
      <div className="overflow-y-auto w-full py-3 flex-1">
        {filteredUsers.map((user) => (
          ...
          >
            ...
            {/* User name and status (hidden when collapsed) */}
            {!collapsed && (
